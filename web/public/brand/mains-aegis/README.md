...
- `web/public/favicon.svg`: single-color 09 Lockup favicon for light browser surfaces.
- `web/public/favicon-dark.svg`: single-color 09 Lockup favicon for dark browser surfaces.
- `web/public/pwa/mains-aegis-icon-*.png`: multi-color 09 Lockup application icons for light surfaces.
- `web/public/pwa/mains-aegis-icon-dark-*.png`: multi-color 09 Lockup application icons for dark surfaces.
...
Platform application PNGs add only a flat theme surface and, for maskable exports, one uniform outer scale to fit the platform safe zone. They do not crop, rotate, mirror, split, or redraw the 09 Lockup. Favicon SVGs remain transparent and single-color. The old standalone `M` icon is not part of the delivery system.
...
The first command writes SVG assets, platform PNGs, and evidence under `output/logo-vector-validation/`, `output/logo-square-lockup-validation/`, and `output/logo-platform-validation/`. The strict command checks vector integrity, path topology, cubic-curve continuity, finite node count, flat-color accuracy, light-theme contrast, fixed-origin reference comparison, the approved square-lockup path identity, the selected 09 wordmark overlay, platform palette identity, maskable safe-zone bounds, banned SVG features, fixed layout geometry, and uncropped `1024px`, `512px`, `256px`, `128px`, and `64px` renders on white, light-Web, and dark-Web surfaces.
