...
- Pass criteria:
  - `START` 前必须可见 `USB-C LOOP CHECK`、`CANCEL` 与 `CONFIRM`；取消不能切换到 active 手动状态。
  - 已确认会话 action bar 显示 `LOOP OK`，但 `LOAD` 的 `USB OUT >=3W` 与 `LOCK` 的 `TPS DATA LOST` 仍以独立 detail 状态可见。
