...
- `BACKUP` 默认显示非充电语义；仅 VIN 已确认无市电的受控 USB-C 低输出会话可显示 runtime `CHG`，不改变顶部 `BACKUP` mode 词形。
...
- `MANUAL` 的 `START` 始终先进入 USB-C 回环确认弹窗；确认后的 action bar status 显示 `LOOP OK`，并且仅在本次 RAM 会话内有效。
