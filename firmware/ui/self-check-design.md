...
| `BQ40Z50` | `PEND/OK/WARN/ERR/N/A` | `SOC xx%` / `ABNORMAL` / `RCA ALARM` / `EMSHUT ACTIVE` / `NOT DETECTED` |
...
- `BQ40Z50` 的放电阻断原因优先级固定为：`OperationStatus()[EMSHUT]` 先显示 `EMSHUT ACTIVE`，再考虑普通 `XDSG BLOCKED` / `DSG FET OFF`。`EMSHUT ACTIVE` 使用同一个 `Recover` 确认入口，但恢复链路会主动执行 BQ40 communication-exit 触发。
...
![Self-check Variant C - BQ40 EMSHUT idle](assets/self-check-c-bq40-emshut-blocked.png)
![Self-check Variant C - BQ40 EMSHUT recovery dialog](assets/self-check-c-bq40-emshut-dialog.png)
