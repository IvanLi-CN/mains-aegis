...
// Keep regular activation on the old 2 s observe cadence, but let discharge authorization exercise
// documented EMSHUT communication-exit promptly once we can already read a blocked gauge.
...
const BMS_DISCHARGE_AUTH_PROBE_WITHOUT_CHARGE_PERIOD: Duration = Duration::from_millis(500);
...
const CHARGER_UNDER_DELIVERY_DIAG_PERIOD: Duration = Duration::from_secs(10);
const USB_PD_CHARGER_DELIVERY_DIAG_PERIOD: Duration = Duration::from_secs(10);
...
fn bq40_physical_discharge_path_absent(
    pack_mv: Option<u16>,
...
fn read_bq40_op_status_with_raw_trace<I2C>(
    i2c: &mut I2C,
    addr: u8,
...
where
    I2C: embedded_hal::i2c::I2c,
{
    let Ok(trace) = bq40z50::read_block_raw_trace(i2c, addr, bq40z50::cmd::OPERATION_STATUS) else {
    ...
    I2C: embedded_hal::i2c::I2c,
{
    Bq40LockDiagSnapshot {
        ...
        .ok()
        .flatten(),
        da_configuration: bq40z50::read_data_flash_u16(
            i2c,
            addr,
        ...
        .ok()
        .flatten(),
        power_config: bq40z50::read_data_flash_u16(i2c, addr, bq40z50::data_flash::POWER_CONFIG)
            .ok()
            .flatten(),
    }
}
...
fn usb_pd_contract_kind_name(kind: usb_pd::ContractKind) -> &'static str {
    match kind {
    ...
    F: FnMut(SelfCheckStage, SelfCheckUiSnapshot),
{
    ...
    let mut charger_vbat_adc_mv: Option<u16> = None;
    ...
    if charger_enabled {
        ...
        charger_vbat_adc_mv = bq25792::read_adc_u16(&mut *i2c, bq25792::reg::VBAT_ADC).ok();
    ...
    if bq40_physical_discharge_path_absent(
        bms_voltage_mv,
        bms_discharge_ready,
...
pub struct PowerManager<'d, I2C> {
    ...
    usb_pd_charger_delivery_diag_next_at: Instant,
    chg_enabled: bool,
    charger_allowed: bool,
    ...
    diag_snapshot: DerivedPowerSnapshot,
    ...
    I2C: embedded_hal::i2c::I2c<Error = esp_hal::i2c::master::Error>,
{
    pub fn new(
        ...
        Self {
            ...
            usb_pd_charger_delivery_diag_next_at: now,
            chg_enabled: false,
            charger_allowed,
            ...
            diag_snapshot: DerivedPowerSnapshot::empty(),
    ...
    fn refresh_bq40_manufacturing_diag_snapshot(&mut self) {
        ...
        self.diag_snapshot.bms.fet_en = bq40_mac_bit(
            lock_diag.manufacturing_status,
            bq40z50::manufacturing_status::FET_EN,
        );
        self.diag_snapshot.bms.chg_en = bq40_mac_bit(
            lock_diag.manufacturing_status,
            bq40z50::manufacturing_status::CHG_EN,
        );
        self.diag_snapshot.bms.dsg_en = bq40_mac_bit(
            lock_diag.manufacturing_status,
            bq40z50::manufacturing_status::DSG_EN,
        );
        self.diag_snapshot.bms.charging_inhibit =
            bq40_mac_bit(charging_status, bq40z50::charging_status::IN);
        self.diag_snapshot.bms.charging_suspend =
            bq40_mac_bit(charging_status, bq40z50::charging_status::SU);
        self.diag_snapshot.bms.charging_hv =
            bq40_mac_bit(charging_status, bq40z50::charging_status::HV);
        self.diag_snapshot.bms.current_at_eoc_ma = lock_diag.current_at_eoc_ma;
        self.diag_snapshot.bms.da_configuration = lock_diag.da_configuration;
        self.diag_snapshot.bms.power_config = lock_diag.power_config;
        self.diag_snapshot.bms.emshut_en = bq40_df_bit(
            lock_diag.da_configuration,
            bq40z50::da_configuration::EMSHUT_EN,
        );
        self.diag_snapshot.bms.emshut_pexit_dis = bq40_df_bit(
            lock_diag.da_configuration,
            bq40z50::da_configuration::EMSHUT_PEXIT_DIS,
        );
        self.diag_snapshot.bms.emshut_exit_comm = bq40_df_bit(
            lock_diag.power_config,
            bq40z50::power_config::EMSHUT_EXIT_COMM,
        );
        self.diag_snapshot.bms.emshut_exit_vpack = bq40_df_bit(
            lock_diag.power_config,
    ...
    fn refresh_derived_power_input_snapshot(&mut self) {
        ...
        self.diag_snapshot.input = DerivedPowerInputSnapshot {
            source: self
                .ui_snapshot
                .dashboard_detail
                .input_source
            ...
            usb_pd_unsafe_source_latched: usb_pd.unsafe_source_latched,
            usb_pd_contract_kind: usb_pd
                .contract
                .map(|contract| usb_pd_contract_kind_name(contract.kind)),
            usb_pd_contract_mv: usb_pd.contract.map(|contract| contract.voltage_mv),
            usb_pd_contract_ma: usb_pd.contract.map(|contract| contract.current_ma),
            usb_pd_vac1_mv: self.usb_pd_vac1_mv,
            usb_pd_vsys_mv: self.usb_pd_vsys_mv,
        };
    }
    ...
    pub fn update_usb_pd_state(&mut self, state: usb_pd::UsbPdPortState) {
        ...
        self.maybe_log_usb_pd_charger_delivery_diag(Instant::now(), state);
        ...
        self.recompute_ui_mode();
        self.refresh_derived_power_input_snapshot();
    }

    fn maybe_log_usb_pd_charger_delivery_diag(
        &mut self,
        now: Instant,
        state: usb_pd::UsbPdPortState,
    ) {
        if now < self.usb_pd_charger_delivery_diag_next_at {
            return;
        }

        let Some(target_ichg_ma) = self.ui_snapshot.bq25792_ichg_ma else {
            return;
        };
        if !self.manual_charge_runtime.active
            && target_ichg_ma < ManualChargeSpeed::Ma1000.ichg_ma()
        {
            return;
        }

        let actual_charge_ma = self
            .ui_snapshot
            .bq25792_ibat_ma
            .or(self.ui_snapshot.bq40z50_current_ma)
            .map(|ma| ma.max(0));
        let Some(actual_charge_ma) = actual_charge_ma else {
            return;
        };
        if i32::from(target_ichg_ma)
            <= i32::from(actual_charge_ma) + i32::from(CHARGER_LIMIT_DIAG_MARGIN_MA)
        {
            return;
        }

        self.usb_pd_charger_delivery_diag_next_at = now + USB_PD_CHARGER_DELIVERY_DIAG_PERIOD;
        let contract_mv = state.contract.map(|contract| contract.voltage_mv);
        let contract_ma = state.contract.map(|contract| contract.current_ma);
        esp_println::println!(
            "usb_pd: charger_delivery_diag reason=charge_under_target_runtime manual_active={} manual_speed_ma={} applied_ichg_ma={:?} ibat_adc_ma={:?} bms_current_ma={:?} input_ibus_ma={:?} contract_mv={:?} contract_ma={:?} pd_vindpm_mv={:?} pd_input_current_limit_ma={:?}",
            self.manual_charge_runtime.active,
            self.manual_charge_prefs.speed.ichg_ma(),
            self.ui_snapshot.bq25792_ichg_ma,
            self.ui_snapshot.bq25792_ibat_ma,
            self.ui_snapshot.bq40z50_current_ma,
            self.ui_snapshot.input_ibus_ma,
            contract_mv,
            contract_ma,
        ...
        defmt::warn!(
            "usb_pd: charger_delivery_diag reason=charge_under_target_runtime manual_active={=bool} manual_speed_ma={=u16} applied_ichg_ma={=?} ibat_adc_ma={=?} bms_current_ma={=?} input_ibus_ma={=?} contract_mv={=?} contract_ma={=?} pd_vindpm_mv={=?} pd_input_current_limit_ma={=?}",
            self.manual_charge_runtime.active,
            self.manual_charge_prefs.speed.ichg_ma(),
            self.ui_snapshot.bq25792_ichg_ma,
            self.ui_snapshot.bq25792_ibat_ma,
    ...
    pub fn request_manual_charge_action(&mut self, action: ManualChargeUiAction) {
        ...
        match action {
            ...
            ManualChargeUiAction::Stop => {
                ...
                self.chg_next_poll_at = now;
                esp_println::println!("manual_charge: stop user_requested=true");
    ...
    fn maybe_log_charger_limit_mismatch(
        ...
        self.chg_limit_diag_next_at = now
            + if diag_kind.is_under_delivery() {
                CHARGER_UNDER_DELIVERY_DIAG_PERIOD
            } else {
                CHARGER_LIMIT_DIAG_PERIOD
            };
        ...
        esp_println::println!(
            "charger: delivery_diag reason={} manual_active={} manual_speed_ma={} policy_target_ichg_ma={:?} applied_ichg_ma={:?} applied_iindpm_ma={:?} raw_ibus_adc_ma={:?} ibat_adc_ma={:?} bms_current_ma={:?} usb_pd_contract_mv={:?} usb_pd_contract_ma={:?} usb_pd_vindpm_mv={:?} usb_pd_input_current_limit_ma={:?} reg03={:?} reg03_ma={:?} reg06={:?} reg06_ma={:?} reg10={:?} reg14={:?} chg_stat={} vbus_stat={} vindpm={} iindpm={} poorsrc={} st0=0x{:02x} st1=0x{:02x}",
            diag_kind.as_str(),
            self.manual_charge_runtime.active,
            self.manual_charge_prefs.speed.ichg_ma(),
            policy_target_ichg_ma,
    ...
    fn maybe_run_bms_activation_probe_without_charge(
        ...
        let now = Instant::now();
        let exercise_exit = self.bms_recovery_requires_discharge_ready();
        ...
        *next_at = now
            + if exercise_exit {
                BMS_DISCHARGE_AUTH_PROBE_WITHOUT_CHARGE_PERIOD
            } else {
                BMS_ACTIVATION_PROBE_WITHOUT_CHARGE_PERIOD
            };
        ...
        let exercise_due = exercise_exit
            && dwell_ms <= BMS_ACTIVATION_EXIT_EXERCISE_WINDOW.as_millis() as u64
            && self
    ...
    fn apply_bq40_activation_snapshot(
        ...
        let (charge_ready, charge_reason) = bq40_decode_charge_path(snapshot.op_status);
        let (discharge_ready, discharge_reason) = bq40_decode_discharge_path(snapshot.op_status);
        let physical_path_absent = bq40_physical_discharge_path_absent(
            Some(snapshot.vpack_mv),
            discharge_ready,
            self.ui_snapshot.bq25792_vbat_present,
        );
        let effective_discharge_ready = if physical_path_absent {
            Some(false)
        } else {
            discharge_ready
        };
        ...
        } else if matches!(effective_discharge_ready, Some(false)) || rca_alarm {
            SelfCheckCommState::Warn
        } else {
            SelfCheckCommState::Ok
        };
        ...
        defmt::info!(
            ...
            rca_alarm,
            charge_ready,
            effective_discharge_ready,
            primary_reason
        );

        self.bms_recovery_result_from_snapshot(state, effective_discharge_ready, low_pack_runtime)
    }
    ...
    fn output_gate_reason_now(&mut self) -> OutputGateReason {
        ...
        if self.bms_addr.is_none()
            ...
            || bq40_physical_discharge_path_absent(
                self.ui_snapshot.bq40z50_pack_mv,
                self.ui_snapshot.bq40z50_discharge_ready,
                self.ui_snapshot.bq25792_vbat_present,
            )
        {
    ...
    fn maybe_poll_charger(&mut self, irq: &IrqSnapshot) {
        if !self.charger_allowed {
            ...
            self.update_manual_charge_ui_snapshot(Instant::now());
            self.refresh_derived_power_input_snapshot();
            self.diag_snapshot.charger.poll_valid = false;
            self.diag_snapshot.charger.enabled = false;
            self.diag_snapshot.charger.allow_charge = false;
            self.diag_snapshot.policy.status = "disabled";
            self.diag_snapshot.policy.notice = "charger_disabled";
            self.recompute_ui_mode();
            return;
        }
        ...
        let raw_vac2_adc_mv = adc_state.and_then(|_| bq25792::read_vac2_adc_mv(&mut self.i2c).ok());
        ...
        if dc_input_selected && !force_allow_charge && !auto_force_charge && !activation_pending {
            ...
            match bq25792::set_input_voltage_limit_mv(&mut self.i2c, target_vindpm_mv) {
                ...
                Err(e) => {
                    self.mark_charger_poll_failed(now);
                    defmt::error!(
                        "charger: bq25792 err stage=dc_vindpm_write err={}",
                        i2c_error_kind(e)
                    );
                    return;
                }
            ...
            match bq25792::set_input_current_limit_ma(
                &mut self.i2c,
                CHARGE_POLICY_DC_INPUT_IINDPM_MA,
            ) {
                Ok(v) => applied_iindpm_ma = Some(decode_cur_ma(v)),
                Err(e) => {
                    self.mark_charger_poll_failed(now);
                    defmt::error!(
                        "charger: bq25792 err stage=dc_iindpm_write err={}",
                        i2c_error_kind(e)
                    );
                    return;
                }
            ...
            match usb_pd_input_limit_update(
                ...
                UsbPdInputLimitUpdate::ApplyContract => {
                    if self.usb_pd_input_limit_backup.is_none() {
                        let vindpm_reg = match bq25792::read_u8(
                            &mut self.i2c,
                            bq25792::reg::INPUT_VOLTAGE_LIMIT,
                        ) {
                            Ok(v) => v,
                            Err(e) => {
                                self.mark_charger_poll_failed(now);
                        ...
                        let iindpm_reg = match bq25792::read_u16(
                            &mut self.i2c,
                            bq25792::reg::INPUT_CURRENT_LIMIT,
                        ) {
                            Ok(v) => v,
        ...
        self.refresh_derived_power_input_snapshot();
        self.diag_snapshot.charger = DerivedPowerChargerSnapshot {
            poll_valid: true,
            enabled: self.chg_enabled,
            ce_low: self.chg_ce.is_low(),
            ilim_hiz_brk_low: self.chg_ilim_hiz_brk.is_low(),
            allow_charge,
            normal_allow_charge,
            force_allow_charge,
            can_enable,
            ...
            adc_done,
            adc_ready,
            ibus_adc_ma: raw_ibus_adc_ma,
            ibat_adc_ma,
            vbus_adc_mv: raw_vbus_adc_mv,
            vbat_adc_mv,
            vsys_adc_mv,
            ...
            ico_stat: bq25792::decode_ico_stat(ico_stat),
            treg,
            dpdm,
            wd,
            poorsrc,
        ...
        self.diag_snapshot.policy = DerivedPowerPolicySnapshot {
            state: policy_state.map(ChargePolicyState::as_str),
            status: policy_status_text,
            notice: policy_notice_text,
            input_source: dashboard_input_source_name(input_source),
            start_reason: policy_start_reason.map(ChargeStartReason::as_str),
            ...
            dc_derated: self.charge_policy_derate.derated,
            output_blocked: self.charge_policy_output_load.blocked,
            manual_active: self.manual_charge_runtime.active,
            manual_stop_inhibit: self.manual_charge_runtime.stop_inhibit,
        };
    ...
    fn mark_charger_poll_failed(&mut self, now: Instant) {
        ...
        self.diag_snapshot.charger.enabled = false;
        self.diag_snapshot.charger.allow_charge = false;
        self.diag_snapshot.policy.status = "error";
        self.diag_snapshot.policy.notice = "charger_poll_failed";
        self.recompute_ui_mode();
    }
        ...
        for (idx, addr) in addr_order.into_iter().take(addr_count).enumerate() {
            match self.read_bq40z50_snapshot_strict(addr) {
                Ok(s) => {
                    ...
                    self.ui_snapshot.bq40z50 = if low_pack
                        || rca_alarm
                        || !matches!(effective_discharge_ready, Some(true))
                    {
                        SelfCheckCommState::Warn
                    } else {
                        SelfCheckCommState::Ok
                    ...
                    self.refresh_derived_power_bms_snapshot(addr, &s);
    ...
    fn refresh_derived_power_bms_snapshot(&mut self, addr: u8, s: &Bq40z50Snapshot) {
        ...
        let gauging_status = s
            .gauging_status
            .or_else(|| lock_diag.and_then(|diag| diag.gauging_status));
        ...
        let da_configuration = lock_diag.and_then(|diag| diag.da_configuration);
        let power_config = lock_diag.and_then(|diag| diag.power_config);
        let (charge_ready, _) = bq40_decode_charge_path(op_status);
        let (decoded_discharge_ready, _) = bq40_decode_discharge_path(op_status);
        let discharge_ready = if bq40_physical_discharge_path_absent(
            Some(s.vpack_mv),
        ...
        self.diag_snapshot.bms = DerivedPowerBmsSnapshot {
            addr: Some(addr),
            state: self_check_comm_state_name(self.ui_snapshot.bq40z50),
            pack_mv: Some(s.vpack_mv),
            current_ma: Some(s.current_ma),
            soc_pct: Some(s.rsoc_pct),
            cell_min_mv: Some(bq40_cell_min_mv(s)),
            cell_max_mv: Some(bq40_cell_max_mv(s)),
            no_battery: self.ui_snapshot.bq40z50_no_battery,
            discharge_ready,
            charge_ready,
            ...
            charging_inhibit: bq40_mac_bit(charging_status, bq40z50::charging_status::IN),
            charging_suspend: bq40_mac_bit(charging_status, bq40z50::charging_status::SU),
            charging_hv: bq40_mac_bit(charging_status, bq40z50::charging_status::HV),
            current_at_eoc_ma: lock_diag.and_then(|diag| diag.current_at_eoc_ma),
            da_configuration,
            power_config,
            emshut_en: bq40_df_bit(da_configuration, bq40z50::da_configuration::EMSHUT_EN),
            emshut_pexit_dis: bq40_df_bit(
                da_configuration,
                bq40z50::da_configuration::EMSHUT_PEXIT_DIS,
            ),
            emshut_exit_comm: bq40_df_bit(power_config, bq40z50::power_config::EMSHUT_EXIT_COMM),
            emshut_exit_vpack: bq40_df_bit(power_config, bq40z50::power_config::EMSHUT_EXIT_VPACK),
        };
    }
...
struct Bq40z50Snapshot {
    ...
    batt_status: u16,
    op_status: Option<u32>,
    op_status_raw_len: Option<u8>,
    op_status_raw_bytes: Option<[u8; 4]>,
