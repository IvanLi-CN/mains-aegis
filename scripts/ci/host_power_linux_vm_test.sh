...
"${ssh_base[@]}" 'sudo env MAINS_AEGIS_DEVD_ALLOW_HOST_POWER_ACTIONS=true nohup /usr/local/bin/mains-aegis-devd serve-http --ipc /tmp/mains-aegis-devd-linux.sock --bind 0.0.0.0:30080 --allow-lan-bridge --auth-token-file /home/ci/mains-aegis-devd.token > /tmp/mains-aegis-devd.log 2>&1 &'
