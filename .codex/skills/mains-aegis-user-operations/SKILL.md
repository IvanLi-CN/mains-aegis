...
- Prefer `mains-aegis` CLI for commands; use `mains-aegis-devd serve` for IPC daemon ownership.
- Use `mains-aegis-devd bridge-http` when Web/API access is explicitly needed; CLI commands can point at its `--ipc` endpoint to share the same daemon state.
- Non-loopback `bridge-http` requires `--allow-lan-bridge` and `--auth-token-file`; API clients must send `Authorization: Bearer <token>`, while browser EventSource requests use `bridge_token=<token>`.
