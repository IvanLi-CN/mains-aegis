---
title: 前面板 UI 交互与设计
description: 前面板屏幕的信息架构、交互路径、状态语义与设计约束。
---

# 前面板 UI 交互与设计

本页描述前面板固件 UI 的结构、交互和状态语义。它回答的不是“有哪些图”，而是“用户上电后会先看到什么、可以点哪里、为什么会停在某一页、每个状态词到底代表什么”。

如果你只是想先看**当前有哪些页面、每页最新长什么样**，请先看 [前面板屏幕页面总览](/design/front-panel-screen-pages)。

## 1. 范围

- 适用对象：样机复刻者、固件协作者、前面板 bring-up 排障者。
- 适用范围：`320x172` 横屏固件 UI；当前页面体系包括 `SELF CHECK`、Dashboard 首页、5 个二级详情页、`MANUAL CHARGE`，以及 `BQ40Z50` 恢复相关 overlay / 结果页。
- 不在本页范围内：Host 侧 UI、网页控制台、营销展示图。

## 2. 交互硬件与设计约束

| 输入/部件 | 事实基线 | 对 UI 设计的约束 |
| --- | --- | --- |
| 电容触摸 | 触摸控制器走 `I2C2_SCL/SDA`，中断独立走 `CTP_IRQ`，不并入 `I2C2_INT` | 页面点击热区可以独立定义，不需要和 `TCA6408A/FUSB302B` 共用一根中断线 |
| 五向按键 | `BTN_UP/DOWN/LEFT/RIGHT` 走 `TCA6408A@0x21`，`BTN_CENTER` 直连 `ESP32-S3 GPIO0` | UI 允许“触摸 + 五向键”并存，但冻结几何主要以触摸热区为准 |
| 中键 `BTN_CENTER` | 直连 `GPIO0`，同时是下载模式 strapping pin | 不能把“按住中键再复位”设计成正常 UI 手势；它首先是硬件启动约束 |
| `TCA_RESET#` | 拉低后 `TCA6408A` 端口回高阻，`CS` 被上拉、`RES/TP_RESET` 被下拉 | 扩展器失控时，屏幕和触摸会一起回到安全态；UI 必须允许“整块前面板被硬复位后重新初始化” |
| 背光 `BLK` | 背光由主控显式控制 | 背光开关不承载业务态，不把“背光是否亮”当作系统状态词 |

设计结论只有一条：**前面板 UI 是诊断界面 + 运行界面，不是纯触摸消费级界面。** 它首先要配合 bring-up 和恢复链路，其次才是交互观感。
...
1. `SELF CHECK` 不是启动动画，而是 bring-up 真相源。
2. Dashboard 不是终点页，它是一个二级仪表盘入口页。

## 4. 页面切换规则

### 4.1 上电先进入 `SELF CHECK`

`SELF CHECK` 是固件屏幕可用后的默认首屏。页面会持续刷新模块状态，只有在自检真正收口后才允许进入 Dashboard。

### 自动进入 Dashboard 的条件

只有以下条件同时满足，UI 才会从 `SELF CHECK` 收口到 Dashboard：
...
这也是为什么有些样机明明“屏能亮、I2C 能通”，页面仍停在 `SELF CHECK`：UI 在明确告诉你**系统尚未满足切入运行态的前提**。
...
- 进度页与结果页不再接受触摸输入，防止用户在恢复链路执行过程中重复触发动作。
- 当前版本**没有**“手动从 `SELF CHECK` 切到 Dashboard”的冻结交互；切页由自检收口条件决定。

### 4.2 Dashboard 首页是 5 个固定入口

Dashboard 首页的职责是“一眼判断当前电源模式”，同时把 5 个子系统暴露成固定热区。

| 首页区域 | 几何（px） | 进入页面 | 为什么这样分 |
| --- | --- | --- | --- |
| 主 KPI 面板 | `x=6 y=22 w=196 h=52` | `Output Detail` | 最左上给输出主路径，符合运行态最常看数据 |
| 次级信息面板 | `x=6 y=76 w=196 h=94` | `Thermal Detail` | 热管理是运行态次高优先级信息 |
...
这里没有做滚动列表，也没有做多页 tab。原因很简单：在 `320x172` 上，**固定五区比可滚动导航更稳定**，而且更容易和样机 bring-up 时的“点击哪里看哪里”对应起来。
...
- 评审时更容易对比不同页是否发生了信息越界

### 唯一的例外：`Charger Detail`

`Charger Detail` 左侧会话区是一个继续向下钻取的热区：

| 区域 | 几何（px） | 跳转 |
| --- | --- | --- |
| charger manual entry | `x=6 y=60 w=150 h=82` | `MANUAL CHARGE` |

也就是说，`CHARGER DETAIL` 不是终点页，而是“运行态只读信息”与“手动充电控制”之间的分界页。

### 4.4 `MANUAL CHARGE` 是操作页，不是状态页

`MANUAL CHARGE` 负责手动充电偏好与运行时控制。它是当前前面板 UI 中唯一明确承担“下发控制动作”的页面。
...
这页刻意把交互集中在底部 action bar，而不是在顶部再放一个返回按钮。原因是：**小屏幕上把返回、状态、执行动作放在一排，更利于盲操作和单手点击**。

## 5. 页面职责与信息分层

### 5.1 `SELF CHECK`

- 责任：说明“哪些模块已经准备好、哪些还在等待、哪些真的坏了”
- 不负责：常规运行监控、长期浏览
- 读法：先看 `BQ40Z50`、`BQ25792`、`TPS55288-A/B`，再看屏幕链路和温度链路

### 5.2 Dashboard 首页

- 责任：给出当前 UPS 模式、主功率信息、充放电与电池健康总览
- 不负责：展开每个子系统的全部字段
- 读法：先看模式，再看左侧主 KPI，最后看右侧三卡

### 5.3 二级详情页

- 责任：把单个子系统讲清楚
- 不负责：跨子系统解释
- 设计规则：字段只属于本页系统，不允许把 `ChargeCard` 的语义搬到 `Output Detail`

### 5.4 `MANUAL CHARGE`

- 责任：只处理“手动充电偏好”和“启动/停止”
- 不负责：展示全部 charger 原始寄存器信息
- 设计规则：设置与动作分离；动作只在底部唯一热区执行
...
- 如果上游 `BMS` 还没有放行，或者 `VBAT` 还无法可信判断，那么 `TPS` 的探测失败会在 UI 上降级成 `WARN`，并给出 `WAIT BMS` 或 `VBAT UNK`
- 只有在上游条件已经满足后，`TPS` 仍然失败，才应该读成真正的 `ERR`

所以 `WAIT BMS` 更像“系统前提未满足”，不是“输出芯片已坏”。
...
这套 UI 不是在追求“像手机 App”，而是在追求**小屏工业界面可读性**。冻结规则如下：
...
如果后续改版破坏了这些规则，那就不是“文案调整”，而是 UI 设计变更。
...
- 想按用户旅程看全页面： [前面板屏幕页面总览](/design/front-panel-screen-pages)
- 想从硬件和运行时基线读起： [前面板与固件](/design/front-panel-and-firmware)
- 想按上电与排障顺序读： [固件烧录与首次自检](/manual/firmware-flash-and-self-test)
...
- [Component contracts](https://github.com/IvanLi-CN/mains-aegis/blob/main/firmware/ui/component-contracts.md)
- [Design language](https://github.com/IvanLi-CN/mains-aegis/blob/main/firmware/ui/design-language.md)
