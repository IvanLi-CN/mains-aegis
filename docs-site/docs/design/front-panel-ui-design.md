...
- 主要写给样机复刻者、固件协作者和前面板 bring-up 排障时看。
...
`MANUAL CHARGE` 负责手动充电偏好与运行时控制。它也是这套前面板 UI 里唯一会直接发起控制动作的页面。
...
- 它不承担常规运行监控，也不适合长期停留浏览。
- 看这页时，先看 `BQ40Z50`、`BQ25792`、`TPS55288-A/B`，再看屏幕链路和温度链路。
