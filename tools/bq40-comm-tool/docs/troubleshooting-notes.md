...
- 在 `tools/bq40-comm-tool` 里落实“最小充电唤醒 + canonical 地址 + 严格校验 + managerd 排障”后，通信已稳定达到验收阈值。
...
- 本轮在线报告中：
  ...
  - `rom_events.flash_attempted=false`
  - `rom_events.flash_done=false`
- 解释：本轮并未进入 ROM 恢复流程，通信稳定性提升来自前述收敛方案，而非重新刷写 BQ 固件。
