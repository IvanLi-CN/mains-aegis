...
- 跟踪：`docs/specs/ina3221-vbus-offset/SPEC.md`（优先以 `U22 IN- -> U22 CHGND` 的口径对比测量）。
...
### 5.2 固件遥测字段（温度与硬停机可见性）

固件会按固定周期输出 `telemetry ...`，并在行尾追加温度与硬停机链路可见性字段（仅追加，不改变既有字段与单位）：

- `tmp_addr=<0x48|0x49>`：对应 `TMP112A` I2C 地址（`out_a→0x48`，`out_b→0x49`）
- `temp_c_x16=<int|err(kind)>`：温度（`°C * 16`，即 `1/16 °C`）；`temp_c = temp_c_x16 / 16`
