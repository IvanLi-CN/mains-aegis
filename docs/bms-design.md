...
关于在电池组 `PACK+ / PACK-` 两端并联**单向 TVS** 的说明：

- TVS 主要用于 ESD/浪涌等**瞬态能量**钳位，并不等价于“反接保护”。
- 当出现反接/异常导致 `PACK+` 相对 `CHGND` 下冲时，单向 TVS 往往会把负压钳在一个“略微为负”的范围（通常接近二极管导通压降量级），这仍属于数据手册所描述的风险触发区间；Q4 的作用是把 Q3 的 `G-S` **直接夹紧**，从机制上避免 Q3 进入线性区受损。
