...
- 输入面板必须直接展示 `source / pressure_state / pressure_score_pct / pressure_reason / vin_baseline_mv / vin_drop_mv`，让 owner 能判断当前 `DC IN` 压力。
- 充电面板必须直接展示 `policy_target_ichg_ma / limit_active / limit_reason / detail_status`，让 owner 能判断充电是否因输入压力而被限流或停充。
...
- USB Console / trace 视图必须把 `kind=event,target=power` 作为可读 power event 显示，而不是只显示原始 JSON blob。
