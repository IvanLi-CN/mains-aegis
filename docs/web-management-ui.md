...
- PWA：Web App 通过 `vite-plugin-pwa` 生成 `manifest.webmanifest`、`sw.js` 与由 09 方形文字 Lockup 派生的多色 192/512 PNG 图标。亮色多色版本是 manifest 的默认安装图标，暗色多色版本作为明确命名的备用导出；maskable 版本只对完整 Lockup 做整体等比缩放以满足安全区。站点 `favicon.svg` 使用同一 09 Lockup 的单色透明 SVG。首次成功打开后，service worker 会预缓存 app shell、Vite 构建产物、Pages fallback、相对 base 深链 navigation helper、PWA 图标和 bundled static firmware artifacts；真实设备 `/api`、`/events`、LAN HTTP/SSE、USB/Web Serial 与 GitHub Release live catalog 不做离线伪造。新版本使用 `registerType="prompt"`：浏览器后台下载新 app shell 后只显示非阻塞更新提示，用户点击 `Update` 并确认后才切换到新版本并刷新页面。安装推荐由 `beforeinstallprompt`/standalone 状态控制：用户在 Fleet 或 Connect 完成首次交互五秒后才出现底部推荐，侧栏保留 `Install app` 命令；iPhone/iPad 提供通用 Add to Home Screen 指引，关闭或拒绝后本地静默 30 天，Demo、设备页、开放对话框和更新提示期间不自动推荐。
