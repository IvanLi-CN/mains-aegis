...
| 电芯连接方式 | 已定 | `4×21700` 电池座（免工具插拔） |
| PACK 端口 ESD/瞬态钳位 | 已定 | `SMBJ18CA`（双向 TVS，`DO-214AA`，并联 `PACK+ / PACK-`） |
| 结构/PCB 约束 | 已定 | 充电器电路 + BMS + 电池座共板（`10 cm × 10 cm` 四层 PCB） |
...
- **输出电压可调范围（能力要求）**：`9–20.8V`（两种固件版本实际只设为 `12V` / `19V`；但器件必须支持 `I2C` 设压，否则不满足项目需求）。
...
| 分块 | 关键器件 | 选型状态 | 说明 | 资料 |
|---|---|---|---|---|
| 数字功放（TDM） | `MAX98357A` | 候选 | PCM 输入的 Class‑D 功放；在本项目固件栈下按 TDM 使用；用于告警/提示音效（不追求高音质，优先低资源占用与易实现） | `docs/audio-design.md` |
| 发声器件 | 小喇叭（`8Ω / 1W`） | 已选 | 目标为“音色比无源蜂鸣器更丰富”；体积优先，典型 `15×11×3mm` 级别 | `docs/audio-design.md` |
...
5. 机械/连接：已确定使用 `4×21700` 电池座，充电器电路 + BMS + 电池座共板（`10 cm × 10 cm` 四层 PCB）；待确认：层叠/铜厚、接口布置、结构件约束？
