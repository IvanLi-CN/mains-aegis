...
- `GPIO19`：USB_D-（USB OTG / USB Serial/JTAG；原理图网名常写 `MCU_DM`，若加入 CH442E 则接 `S1B`）
- `GPIO20`：USB_D+（USB OTG / USB Serial/JTAG；原理图网名常写 `MCU_DP`，若加入 CH442E 则接 `S1C`）
...
| 17 | 预留 | — | 可用于后续项目功能 |
| 18 | 预留 | — | 可用于后续项目功能 |
| 19 | 已分配 | USB_D- | USB 下载调试（不得复用；原理图网名常写 `MCU_DM`，若加入 CH442E 则接 `S1B`） |
| 20 | 已分配 | USB_D+ | USB 下载调试（不得复用；原理图网名常写 `MCU_DP`，若加入 CH442E 则接 `S1C`） |
