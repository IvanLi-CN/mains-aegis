...
- `GPIO6`：`BMS_BTP_INT_N`（`BQ40Z50-R2.BTP_INT`；经 `NMOSFET` 取反后的 BMS 中断线）
- 通信：`BQ40Z50-R2` 挂载在 `I2C1`（`GPIO35/GPIO36`）

说明：

- `BQ40Z50-R2.BTP_INT` 的**有效电平极性可配置**（可配置为高有效或低有效）；但本项目中 `GPIO6` 看到的是经过 `NMOSFET` **硬件反相**后的信号，所以网络名使用 `BMS_BTP_INT_N` 来特指该反相后的节点（而非描述 `BQ40Z50-R2.BTP_INT` 的可配置极性）。
...
- `GPIO41`：`UPS_IN_CE`（**开漏**控制：通过 `NX7002BKWX` 将输入侧 `EN` 控制节点**拉低以禁用**；释放（高阻）后由电阻网络决定 `EN` 电平）
- `GPIO42`：`IN_PG`（输入侧 Power‑Good 指示；原理图来自 `TPS2490.PG`）
...
- `IN_PG` 为输入侧状态信号；固件侧通常按“输入有效/无效”做保护策略（例如限制后级启动/上报告警/降额）。
...
| 6 | 11 | 已分配 | `BMS_BTP_INT_N` | BMS 中断线（`BQ40Z50-R2.BTP_INT` 经 `NMOSFET` 取反；有效极性可配置） |
...
| 41 | 47 | 已分配 | `UPS_IN_CE` | UPS 输入侧 `EN` 控制（开漏；拉低禁用；经 `NX7002BKWX` 下拉 `EN` 节点） |
| 42 | 48 | 已分配 | `IN_PG` | 输入侧 Power‑Good 指示（来自 `TPS2490.PG`） |
