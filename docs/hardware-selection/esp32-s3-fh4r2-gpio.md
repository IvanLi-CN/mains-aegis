...
- 由于面板侧未放置 `TCA_RESET#` 上拉/下拉，主板侧必须定义该节点的默认电平。
- 当前主板在该网络上提供了 `100kΩ` 上拉到 `3V3`；实测表明仅依赖弱上拉并让 MCU 端高阻释放时抗扰裕量不足，因此当前固件推荐将该 GPIO 配置为 **推挽输出**：正常运行时主动驱高，需要复位时拉低一段时间。

### I2C1（主总线，25kHz，3 个 GPIO，封装 36~38 连续）
...
- `GPIO16`：`CHG_ILIM_HIZ_BRK`（`ILIM_HIZ`“刹车”控制：驱动 NMOS 下拉 `BQ25792.ILIM_HIZ` 到 GND，使其进入非开关模式；默认低电平不刹车；**硬件侧增加 `100kΩ` 下拉到 `GND`，确保上电/复位期间不会误触发刹车**）
...
- `BLK` 用于背光电源开关（前面板 `Q16` 栅极控制）；面板侧不放置上拉/下拉，默认由固件在 MCU 正常运行时主动控制。若睡眠模式需要保持背光状态，固件需确保休眠期间该 GPIO 维持期望电平（Deep-sleep 仅 RTC 域 GPIO 可控，需按 Espressif 约束选型/策略）。
...
| BMS 告警/中断 | 1 | `GPIO21` |
| I2C1（25kHz） | 3 | `GPIO48`、`GPIO47`、`GPIO33` |
...
| 1 | 6 | 已分配 | `TCA_RESET#` | 前面板 `TCA6408A.RESET`（低有效）；面板侧无上拉；主板侧有 `100kΩ` 上拉到 `3V3`；当前板级实现建议推挽输出 |
...
| 16 | 22 | 已分配 | `CHG_ILIM_HIZ_BRK` | `ILIM_HIZ` 刹车控制（驱动 NMOS 下拉；建议加 `100kΩ` 下拉确保默认不刹车） |
...
| 48 | 36 | 已分配 | `I2C1_SDA` | I2C1（25kHz）相关；封装 `36~38` 连续块 |
| 47 | 37 | 已分配 | `I2C1_SCL` | I2C1（25kHz）相关；封装 `36~38` 连续块 |
