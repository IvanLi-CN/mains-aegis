...
### 前面板 USB2 Power‑Good（HUSB305‑01，1 个 GPIO）

- `GPIO1`：`USB2_PG`（来自 `HUSB305‑01`；已外接上拉）
...
### I2C1（主总线，1MHz，3 个 GPIO，封装 36~38 连续）
...
- `GPIO34`：`FAN_VSET_PWM`（PWM 输出→RC 滤波后得到风扇控制电压/控制信号）
- `GPIO35`：`FAN_TACH`（转速输入；典型为开漏/开集输出）
- `GPIO36`：`FAN_EN`（风扇电源/驱动使能）
...
- `GPIO38`：`INA3221_WARNING`（`INA3221.WARNING`；开漏/电平型告警：平均值告警，超限拉低）
- `GPIO39`：`INA3221_CRITICAL`（`INA3221.CRITICAL`；开漏/电平型告警：单次转换/可配置求和超限，超限拉低）
...
| 前面板 USB2 PG | 1 | `GPIO1` |
| BMS 告警/中断 | 1 | `GPIO21` |
| I2C1（1MHz） | 3 | `GPIO48`、`GPIO47`、`GPIO33` |
...
| 1 | 6 | 已分配 | `USB2_PG` | 前面板 USB2 `PG` 信号（来自 `HUSB305‑01`；已外接上拉） |
...
| 48 | 36 | 已分配 | `I2C1_SDA` | I2C1（1MHz）相关；封装 `36~38` 连续块 |
| 47 | 37 | 已分配 | `I2C1_SCL` | I2C1（1MHz）相关；封装 `36~38` 连续块 |
| 33 | 38 | 已分配 | `I2C1_INT` | I2C1 中断汇总（开漏线与；`INT` 放最后）；封装 `36~38` 连续块 |
| 34 | 39 | 已分配 | `FAN_VSET_PWM` | PWM 输出→RC 滤波后得到风扇控制电压/控制信号 |
| 35 | 40 | 已分配 | `FAN_TACH` | 转速输入；典型为开漏/开集输出 |
| 36 | 41 | 已分配 | `FAN_EN` | 风扇电源/驱动使能 |
| 37 | 42 | 已分配 | `INA3221_PV` | `INA3221.PV`；开漏/电平型告警（欠压时拉低）；`VPU=3.3V` |
| 38 | 43 | 已分配 | `INA3221_WARNING` | `INA3221.WARNING`；开漏/电平型告警（平均值告警，超限拉低） |
| 39 | 44 | 已分配 | `INA3221_CRITICAL` | `INA3221.CRITICAL`；开漏/电平型告警（单次转换/可配置求和超限，超限拉低） |
