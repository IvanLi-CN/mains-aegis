...
- 在当前主板实现仅依赖 MCU 侧定义高电平的前提下，推荐固件将该 GPIO 配置为 **推挽输出**：正常运行时主动驱高，需要复位时拉低一段时间；若后续硬件补入明确的外部上拉，再评估是否改回开漏释放方案。
...
| 1 | 6 | 已分配 | `TCA_RESET#` | 前面板 `TCA6408A.RESET`（低有效）；面板侧无上拉；主板侧需定义默认高电平；当前板级实现建议推挽输出 |
