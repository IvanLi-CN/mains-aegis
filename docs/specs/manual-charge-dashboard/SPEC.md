...
- `docs/specs/manual-charge-dashboard/`
  - 记录功能规格、验证与视觉证据
...
全局 EEPROM map 由 `docs/specs/eeprom-storage-layout/SPEC.md` 维护。本规格只定义手动充电偏好的持久化语义与 `ManualChargePrefsRecordV1` 字段。

当前手动充电偏好由 `StorageRecordTableV1` 指向 `0x0040` 的 `32B` record。其它 EEPROM slot 的占用与扩展规则以 eeprom-storage-layout 为准。
