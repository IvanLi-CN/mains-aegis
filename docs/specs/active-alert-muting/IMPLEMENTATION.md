...
- `web/src/app/App.tsx`
  ...
  - 单设备次级页沿用当前设备上下文：记录级 transport error 与保留 telemetry 的状态优先显示，当前 discovery 快照仍是临时设备路由的权威来源；若临时 registry 已有明确 transport failure，则将失败状态覆盖到当前 discovery 快照；已保存记录恢复 connected 时清除过期 transport failure、stream error 与旧 telemetry；在线 transport 的命令失败保留为动作反馈，不冒充连接中断。
  - `DeviceRegistry` 将 charge-control detail 与无副作用 preview 的 HTTP、devd、串口失败标记为 `read`，使读取失败保持 transport/data-error 语义；读取成功后恢复 online/stream 状态和 devd/serial connected 标记；实际写入命令继续显示动作错误。
  - DeviceRegistry 的告警相关回读与通道切换使用操作/读取代次 fence；旧 SSE 恢复、serial/devd 回读和延迟结果不会覆盖新通道或当前实例。
