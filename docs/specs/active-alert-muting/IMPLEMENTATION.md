...
- `web/src/app/App.tsx`
  ...
  - 单设备次级页沿用当前设备上下文：记录级 transport error 与保留 telemetry 的状态优先显示，当前 discovery 快照仍是临时设备路由的权威来源；若临时 registry 已有明确 transport failure，则该失败状态优先于在线 discovery 快照；在线 transport 的命令失败保留为动作反馈，不冒充连接中断。
