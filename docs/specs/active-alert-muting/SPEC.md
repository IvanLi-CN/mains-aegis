...
- 设备页面的离线、连接中和传输错误上下文必须优先于保留的旧 telemetry；记录级 transport error 同样必须显示该上下文。次级页面可显示紧凑状态提示，但不得把非在线数据标为 Live；临时设备路由在当前 discovery 快照中仍以该快照为对象权威，但 registry 已记录明确 transport failure 时只能覆盖其失败状态；在线 transport 的命令失败应保留在当前 discovery entry 上作为动作错误反馈，不得误报为连接中断。
