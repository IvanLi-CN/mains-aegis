...
- 2026-06-05: `/api/v1/status` 的 `battery` snapshot 增加四节 `cell_mv` 与 `charge_fet_on` / `discharge_fet_on` / `precharge_fet_on`，Web 电池页可直接展示 per-cell voltage 与三路 BMS MOS 状态，不再依赖 `power-diag` 详情端点。
