...
- `ASSIST` 的 non-charging mode 边界由 `docs/specs/runtime-mode-switching/SPEC.md` 接管；`BACKUP` 默认也停充，但本规格拥有唯一受控例外：`mode=backup`、VIN 已确认无市电、当前源为可充电 USB-C PD 且既有 BMS/温度/输入安全门通过时，自动策略可在低输出功率下保持 `500mA` 充电。
...
- 主固件不自动修改 `BQ40Z50` Data Flash、JEITA 曲线或 termination current 校准；`CUV Recovery=2550mV` 与 `CUV_RECOV_CHG=0` 只由显式维护工具/DF baseline 承接。针对重复 `OC/LOCK` 的顶充终止对齐例外由 `/Users/ivan/Projects/Ivan/mains-aegis/docs/specs/bq40-lock-root-cause/SPEC.md` 单独承接。
