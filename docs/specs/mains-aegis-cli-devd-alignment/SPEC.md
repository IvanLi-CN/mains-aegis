# Mains Aegis CLI / devd alignment

> 当前有效规范以本文为准；实现覆盖与当前状态见 `./IMPLEMENTATION.md`，关键演进原因见 `./HISTORY.md`。

## 接管说明

- 本规格记录的是 host-tools single-crate、CLI-managed devd lifecycle、IPC-only daemon、显式 HTTP/Web service 与 release/install 对齐。
- 其中 CLI `device session` 用户命令面、`session/safeSettings` 历史模型，以及新的 `connection / identity / status / settings / trace` 查询面，已转由 [`lan-management-convergence`](../lan-management-convergence/SPEC.md) 接管。
...
- `device diag-snapshot` 与 devd HTTP/IPC 保持现有调用面并识别 diagnostic schema v2。USB 固件多帧响应由 devd 按 request id、package 顺序和 end 状态聚合；缺帧、重复帧、乱序与超时必须返回结构化协议错误。fresh I2C package 的 `i2c_nack_address`、`i2c_nack_data` 与 `i2c_nack_unknown` 必须原样透传，不得归并为通用 `i2c_nack`；TPS55288 `VREF` 的数据阶段失败与同包后续寄存器的地址阶段失败必须同时保留。
...
- `mains-aegis` CLI 只能通过系统原生 IPC 调用 devd；Unix 使用 Unix domain socket，Windows 使用 named pipe。CLI 不得使用 HTTP、TCP、localhost URL 或 HTTP service 作为 devd 通信路径。
- `mains-aegis` CLI 覆盖设备、artifact、flash dry-run/reset/monitor、serial lease、settings 与 host power 命令族；历史 `device session` 命令面由 lan-management-convergence 替换为 `connection / settings / trace`。
...
- `mains-aegis daemon serve [--ipc <endpoint>] [--idle-timeout-secs <n>] [--allow-host-power-actions]` 在前台启动 IPC daemon，仅作为开发/调试入口。
- `mains-aegis daemon serve` 不接受 `--bind`、`--open-browser` 或其他 HTTP 相关参数。
- `mains-aegis daemon http [--ipc <endpoint>] --bind 127.0.0.1:30080 [--allow-dev-cors] [--open-browser]` 在前台启动显式 HTTP/Web 服务，并在同一进程内启动共享状态的 IPC listener。
- release 包内的 `mains-aegis-devd` daemon binary 可保留对应的 internal foreground 能力，但不作为普通用户或项目 Justfile 的默认入口。
- 默认 hosted 模式下，`mains-aegis daemon http` 托管嵌入式 `web/dist` 产物，并以 same-origin 方式同时提供 `/` 和 `/api`；根路径保持 Fleet Home，不再跳到 `/connect`。
...
- `mains-aegis daemon http --allow-dev-cors` 进入 API-only 开发模式：仅允许 loopback HTTP development origins（`localhost`、`127.0.0.1`、`[::1]`，任意端口）跨源访问 `/api`，不托管嵌入式 hosted app，且不得与 `--open-browser` 组合。
- `mains-aegis daemon http` 绑定非 loopback 地址时必须同时传入 `--allow-lan-bridge` 与 `--auth-token-file <file>`；API 请求额外接受 `Authorization: Bearer <token>`，浏览器 EventSource 兼容 `service_token=<token>` 与 legacy `bridge_token=<token>` query 参数。
- `GET /api/v1/bootstrap` 必须在 token 模式下保持免认证，用于浏览器判断当前 HTTP service 是否需要额外 bearer token；响应的 `token_required` 必须反映当前 HTTP service 配置。
- host-tools Rust 构建只消费预构建的 `web/dist` 嵌入产物；若缺失，构建必须明确失败并提示开发者先执行 `bun run build`。

### CLI commands

- CLI 全局支持 `--ipc <endpoint>`，默认 endpoint 与 devd 一致；工程 Justfile 默认显式使用 repo-local `.tmp/devd.sock`。
...
- CLI v1 覆盖：
  ...
  - `daemon serve|http`
  - `devices list|scan`
  - `device <id> bind|unbind|connect|disconnect|identity|status|diag-snapshot|recovery bms-discharge-authorization|settings|trace|artifact get|artifact select|flash|reset|monitor start|monitor stop`
...
- Web dev proxy 默认指向 `http://127.0.0.1:30080`，可通过 `MAINS_AEGIS_DEVD_URL`、`VITE_DEFAULT_DEVD_URL` 或 `VITE_DEVD_API_BASE` 指向显式启动的 `mains-aegis daemon http --allow-dev-cors`；该地址代表显式启动的开发 HTTP service，不是默认 daemon。需要 Web 与 CLI 共用状态时，CLI 连接同一个 `daemon http --ipc <endpoint>`；CLI 与 devd 的通信仍只能通过系统原生 IPC endpoint，不能连接 Web proxy、HTTP service URL 或任何 TCP 地址。
...
- `mains-aegis daemon http --allow-dev-cors` 只允许 loopback HTTP development origins（`localhost`、`127.0.0.1`、`[::1]`，任意端口），用于 Vite 租约端口或直接 dev API 调试；非 loopback HTTP service 仍必须走 token-gated LAN bridge 规则。
...
- 跨 `Web App` / `mains-aegis-devd` / `mains-aegis` CLI 的通信方案优先级矩阵由 [`client-transport-priority`](../client-transport-priority/SPEC.md) 统一定义；本规格只记录 host-tools crate、IPC/HTTP 命令面与 release/install 对齐，不再重复定义跨客户端优先级表。
...
- `mains-aegis daemon serve --help` 中不出现 `--bind`。
- `mains-aegis daemon http --bind 0.0.0.0:30080` 在缺少 token 文件时失败。
- `mains-aegis daemon http --allow-dev-cors --open-browser` 参数冲突并失败。
- `mains-aegis --ipc <missing.sock> health` 能自动启动 sibling IPC daemon，等待 `devd.health` 成功后返回 health 响应。
- `mains-aegis --ipc <missing.sock> --no-auto-start health` 失败且不得启动 daemon。
- `mains-aegis daemon serve --ipc <endpoint> --idle-timeout-secs 0` 前台运行 IPC daemon；`mains-aegis daemon http --ipc <endpoint> --allow-dev-cors` 前台运行 HTTP/Web service。
...
- 文档、skills 与 AGENTS 不再把直接运行 `mains-aegis-devd`、`tools/mains-aegis-devd` 或 `serve --bind` 当作当前普通入口，并明确本仓 Codex 默认路由为 `$mains-aegis-devd-flow`；`$mains-aegis-user-operations` 仅作为显式 end-user/released-tool 路径。
