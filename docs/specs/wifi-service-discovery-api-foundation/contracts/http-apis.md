...
- `device_id` 为可选的顶层身份字段。新固件应返回逻辑设备 ID；客户端必须校验其与已绑定设备一致。旧固件可能省略该字段，客户端应继续解析 payload，并通过已验证端点与请求代次栅栏避免接受过期结果。
...
- `status` 事件的 `data` 复用普通状态响应的可选 `device_id` 字段；缺失该字段表示旧版固件兼容模式，不改变事件名或其他字段语义。
