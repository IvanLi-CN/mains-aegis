# Web management UI History（web-management-ui）
...
- companion-LAN 的默认直连优先级改为统一引用 [`client-transport-priority`](../client-transport-priority/SPEC.md)；本规格只保留 Web 侧保存字段与交互约束，不再重复维护跨客户端矩阵。
...
- 将 public Demo 入口从 `seed=` 深链切换为 `demo=true`；具体 mock 场景改由页面内 Demo 控制面板选择，避免 URL 参数继续承载产品演示状态。
