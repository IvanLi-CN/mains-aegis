...
- 单设备深链的未知 section 归一化到 Overview；设备加载或暂时不可用时仍保留当前路由的 `h1`，并在次级页用紧凑上下文明确 Connecting、Offline 或 Connection error。记录级 transport error 也必须触发该上下文，不得把保留的旧 telemetry 标为 Live。仅由当前 devd discovery 暂时纳管的设备在后续快照消失后不得继续作为 Fleet 或路由设备；设备仍在当前快照时，即使已完成 hydration 也必须以最新 discovery entry 作为对象权威，但 registry 已记录的明确 transport failure（error、error stream 或已断开的 serial）只能覆盖其失败状态；在线 transport 的命令失败保留在当前 entry 上并显示为动作错误，不得误报为连接中断；首个 status 尚未到达时显示 Waiting。
