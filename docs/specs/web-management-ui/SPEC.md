...
- 本规格完成的是 Web 管理端 v1 基线；当前 LAN 设置语义与 hosted Connect 收敛以 [`lan-management-convergence`](../lan-management-convergence/SPEC.md) 为准，本规格保留 UI foundation 与 USB / devd 会话约束的历史记录。
...
- Web 的默认直连选择统一遵循 [`client-transport-priority`](../client-transport-priority/SPEC.md)；本规格不再重复定义 `hostname_fqdn > hostname > ip:port` 矩阵，只要求未确认 companion 时不得把 pending candidate 自动提升为默认连接路径。
...
- 单设备深链的未知 section 归一化到 Overview；设备加载或暂时不可用时仍保留当前路由的 `h1`，并在次级页用紧凑上下文明确 Connecting、Offline 或 Connection error。记录级 transport error 也必须触发该上下文，不得把保留的旧 telemetry 标为 Live。仅由当前 devd discovery 暂时纳管的设备在后续快照消失后不得继续作为 Fleet 或路由设备；在线 transport 的命令失败显示为动作错误，不得误报为连接中断；首个 status 尚未到达时显示 Waiting。
...
- Web 端以 `docs/specs/wifi-service-discovery-api-foundation/contracts/http-apis.md` 为只读 API 契约来源。
