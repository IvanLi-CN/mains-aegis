...
- 单设备深链从当前 devd discovery 暂时纳管的记录只在该发现快照仍包含设备时参与 Fleet 与路由选择；hydration 后仍以最新 discovery entry 为准，设备消失后保留路由标题并进入不可用态。若临时 registry 记录已经记录明确 transport failure（error、error stream 或已断开的 serial），则把失败状态覆盖到当前 discovery entry，避免在线 discovery 快照覆盖错误上下文；在线 transport 的命令错误仍附着在当前 discovery entry 并显示为 `Action failed`，不冒充连接中断；无首个 status 时显示 `Waiting`。
