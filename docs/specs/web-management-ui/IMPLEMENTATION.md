...
- 单设备深链从当前 devd discovery 暂时纳管的记录只在该发现快照仍包含设备时参与 Fleet 与路由选择；hydration 后仍以最新 discovery entry 为准，设备消失后保留路由标题并进入不可用态。在线 transport 的命令错误显示为 `Action failed`，不冒充连接中断；无首个 status 时显示 `Waiting`。
