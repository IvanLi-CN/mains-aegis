...
- 默认把运行时 SPI 频率提升到 `20MHz`，保留 `40MHz` 实验开关。
...
- `firmware/Cargo.toml`
  - `esp-hal` 启用 `psram`，增加 `display-spi-40mhz` 实验 feature。
...
- GC9307 初始化阶段保留 `10MHz`；初始化完成后切换到 `20MHz` runtime SPI。启用 `display-spi-40mhz` feature 时，runtime SPI 提升到 `40MHz`。
...
- `FrontPanel` 默认通过 `SPI2 + DMA_CH1 + PSRAM` 驱动屏幕；runtime SPI 默认频率为 `20MHz`。
...
- 实验 feature `display-spi-40mhz` 保留在实现与文档中，但不作为默认构建配置。

## 实现前置条件（Definition of Ready / Preconditions）

- 目标、范围与 20/40MHz 策略已冻结。
- 以 PSRAM 为正式依赖的前提已确认。
...
- Hardware manual validation: `20MHz` 基线验证；`40MHz` 作为实验配置保留待板级联调。
...
- [x] M4: 主固件与 `test-fw` 切换到 `PSRAM + DMA_CH1` 显示主路径，默认运行时 `20MHz` 并保留 `40MHz` 实验开关。
