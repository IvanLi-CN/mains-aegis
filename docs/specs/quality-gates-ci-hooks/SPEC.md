# 仓库代码质量门槛：Git hooks + GitHub Actions
...
- 不在本规格内修改任何固件业务逻辑或硬件相关 bring-up 流程。
- 不在本规格内引入发布/制品/签名/OTA 流水线。
- 不在本规格内新增全套测试体系；仅冻结质量门槛的最低闭环（format/lint/build 等）。
...
- 任何与硬件连接/烧录/运行相关的自动化（硬件依赖步骤仍为手工验证或后续规格）。
...
- 已完成必要的 repo reconnaissance（本规格已冻结目标文件路径与工作流职责分工）

## 非功能性验收 / 质量门槛（Quality Gates）

### Testing

- Unit tests: 本规格不新增；以“门槛自动化”闭环为主。
...
- 假设（已冻结）：
  ...
  - 固件工程目录将与 `esp-rs-no-std-firmware-bootstrap` 的实现结果保持一致（例如存在 `firmware/`），CI 的 path-filter 将以此为准。
