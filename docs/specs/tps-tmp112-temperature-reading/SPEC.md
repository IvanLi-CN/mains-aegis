...
- 本规格要落地“TPS 边上的 TMP（TMP112A）”温度读取，并将读数**追加到**既有的 `telemetry ...` 输出行中（不改变原字段与单位），以便 bring-up 与回归时一眼能把电压/电流/温度对齐观察。
...
- 不在本规格内实现过温控制闭环（降额/关断/自动恢复策略）；只交付“读数 + 观测口径”。
- 不在本规格内实现/冻结 `TMP112A.ALERT` 的阈值配置与去抖策略；`THERM_KILL_N` 的硬件链路只做“可见性补充”（本规格**仅要求读取并上报** `therm_kill_n`，不做控制闭环）。
- 不在本规格内与上位机协议对接（HTTP/RPC/文件落盘等）；仅定义固件串口/日志口径。
...
- 任何对 `tps55288-control` 已冻结规格文档的修改（本规格只在自身契约中定义“字段追加规则”，保证兼容）。
...
- `docs/power-monitoring-design.md`: 若本规格最终冻结了日志口径与单位，补充“固件上报字段名/单位”小节引用 `tps-tmp112-temperature-reading`（避免文档与实现口径漂移）。
...
- 风险：
  ...
  - `TMP112A` 读数代表“板级热点”而非结温；阈值策略若未来加入，需要样机热测试回归（不在本规格内）。
- 假设（需主人确认）：
  - None
...
- `docs/specs/tps55288-control/SPEC.md`（I2C1 共用与遥测口径相关）
