...
- 不在本计划内实现/冻结 `TMP112A.ALERT` 的阈值配置与去抖策略；`THERM_KILL_N` 的硬件链路只做“可见性补充”（本计划**仅要求读取并上报** `therm_kill_n`，不做控制闭环）。
...
- 任何对 `tps55288-control` 已冻结计划文档的修改（本计划只在自身契约中定义“字段追加规则”，保证兼容）。
...
- `docs/power-monitoring-design.md`: 若本计划最终冻结了日志口径与单位，补充“固件上报字段名/单位”小节引用 `tps-tmp112-temperature-reading`（避免文档与实现口径漂移）。
...
- `docs/specs/tps55288-control/PLAN.md`（I2C1 共用与遥测口径相关）
