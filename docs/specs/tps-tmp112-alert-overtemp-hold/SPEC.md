# TMP112A 过温告警输出：Comparator 模式保持输出
...
- 缺口：固件目前仅做温度读数与 `THERM_KILL_N` 可见性上报（`tps-tmp112-temperature-reading`），但未冻结/落地 `TMP112A.ALERT` 的阈值与模式配置；若依赖上电默认值，告警语义可能与“项目设计（过温时保持输出）”不一致。
