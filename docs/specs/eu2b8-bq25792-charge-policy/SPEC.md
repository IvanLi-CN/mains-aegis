...
- BMS CUV 低电恢复允许在 `PCHG=true`、`PF=0`、`CUVC=false` 且 DC IN 在线时进入受控 `100mA` 预充；该恢复态显示为 `CHG100`，但不同于 DC 过流降档。
...
- `charging_100ma_bms_recovery` 期间，BQ25792 的 `termination_done` 不能作为满充依据；策略必须保持 `full_latched=false`，直到 BMS 离开恢复窗口并重新满足常规满充定义。
...
- 假设：常规充电中 `BQ40 FC` 与 `BQ25792 termination_done` 任一成立即可视为满充停充；CUV recovery 预充态下 `termination_done` 只作为 charger 状态观测值，不触发满充锁存。
