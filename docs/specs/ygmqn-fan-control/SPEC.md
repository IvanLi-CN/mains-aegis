...
- 若 `FAN_VSET_PWM` 初始化失败，固件必须退化到“`FAN_EN` 常开”的保守散热模式，而不是静默失去风扇控制。
...
- 2026-03-13: 已落地 `esp_firmware::fan` 纯逻辑状态机、`FAN_TACH/FAN_EN/FAN_VSET_PWM` 固件接线、`fan:` 日志与 bench 文档；补充 `tach` 故障锁存与 PWM 初始化失败强制 `FAN_EN` fail-safe；PR 为 `#36`，当前等待 review-loop 收敛。
