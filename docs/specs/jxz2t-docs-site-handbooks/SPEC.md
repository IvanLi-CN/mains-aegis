...
- 需要新增一个公开文档站，把“设计手册”与“复刻与使用手册”统一发布，并在不重造事实源的前提下，把现有专题文档组织成更可读的入口。
...
- 站点只保留中文单语导航，主导航固定为“首页 / 设计手册 / 复刻与使用 / GitHub”。
- 重写一组面向外部读者的手册页，把现有 `docs/**`、`docs/pcbs/**`、`firmware/README.md` 等资料整理成清晰的入门路径。
...
- 新增 Rspress 配置、品牌资源、首页、404 与两套手册页面。
...
- 站点公开路由必须包含：`/`、`/design/`、`/design/system-overview`、`/design/power-and-bms`、`/design/front-panel-and-firmware`、`/manual/`、`/manual/prepare-and-scope`、`/manual/pcb-and-wiring-checks`、`/manual/firmware-flash-and-self-test`、`/manual/basic-use-and-troubleshooting`。
...
- 协作者打开 GitHub Pages 首页时，应立即理解项目定位、当前公开范围，以及应该先读设计手册还是复刻手册。
- 进入“设计手册”后，应能快速看到系统概览、电源/BMS 设计、前面板与固件边界，并进一步跳转到仓库中的专题文档。
- 进入“复刻与使用”后，应能依次获得：准备前提、PCB 与连线检查、固件构建烧录与首次自检、基础使用与常见排障。
...
- 若没有完整 BOM / Gerber 打包事实源，复刻手册必须明确说明这一边界，并把读者导向现有 `docs/pcbs/**` 与设计文档，而不是编造缺失内容。
...
- Browser validation: 首页、设计手册首页、复刻与使用手册首页、至少一页关键内容页截图。
...
![设计手册首页](./assets/design-home.png)

复刻与使用首页

![复刻与使用首页](./assets/manual-home.png)

系统概览页

![系统概览页](./assets/system-overview.png)
...
- 复用 `octo-rill` 的独立 `docs-site/` 模式，把公开文档站与仓库事实源分层：站点负责对外叙事与入口，仓库原文负责深度事实。
...
- 风险：仓库中没有完整 BOM/生产文件总表，复刻手册必须清楚写明“当前事实范围”，否则容易误导读者。
- 风险：若 GitHub Pages 权限未启用，workflow 只能构建不能实际部署。
- 需要决策的问题：None。
- 假设（已确认）：站点只公开双手册，不公开 vendor 资料库导航。

## 变更记录（Change log）

- 2026-04-08: 新建规格，冻结双手册文档站的范围、路由与验收口径。
