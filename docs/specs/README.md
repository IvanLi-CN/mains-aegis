...
| f3c2g | Dashboard detail drill-down | 已完成 | `f3c2g-dashboard-detail-drilldown/SPEC.md` | 2026-03-15 | 首页 5 区点击进入二级仪表盘；已补齐详情页 UI 壳层、触摸路由与评审图 |
...
| h43mk | Main firmware runtime audio cues | 已完成 | `h43mk-main-firmware-runtime-audio-cues/SPEC.md` | 2026-03-15 | 主固件已切换到运行时 cue 服务；市电音效判定现统一使用 `DC5025 VIN>=3V`，不再受 charger `input_present` 抖动影响 |
...
| eu2b8 | BQ25792 500mA charge policy + DC derate | 部分完成（4/5） | `eu2b8-bq25792-charge-policy/SPEC.md` | 2026-03-28 | 主线已落地 `80% / 3.70V` 启停、满充锁存、`DC5025` 独占输入 `3.0A -> 100mA` 降档、`VIN`/`TPS>5W` 停充门控、`IBAT_ADC` 实测显示与 host-side 预览场景；等待最终视觉证据晋升与 PR 收口 |
