...
| Front panel auto sleep | active | [current](front-panel-auto-sleep/IMPLEMENTATION.md) | [SPEC](front-panel-auto-sleep/SPEC.md) | - | 主固件使用正式 `180s / 240s / 245s` 自动低亮、关背光与 GC9307 sleep；压缩时序仅用于纯逻辑测试 |
...
| Web management UI | active | [current](web-management-ui/IMPLEMENTATION.md) | [SPEC](web-management-ui/SPEC.md) | - | Web 管理端 v1 基线与 Fleet Summary / Device Overview 页面归属已收口，LAN 管理、settings 与 LAN/USB 收敛改由 lan-management-convergence 接管 |
