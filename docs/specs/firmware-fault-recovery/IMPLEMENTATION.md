...
- Safe mode requests no outputs during self-test and best-effort disables both TPS channels before self-test; PowerManager applies the same clamp again after construction.
...
- Normal release `886658d3-clean-864b4cf9abb0304d` returned a parseable fresh native-CDC status with `reset_cause=power_on`, `abnormal_boots=0`, `safe_mode=false`, truthful single-image rollback diagnostics, and live input/output measurements.
