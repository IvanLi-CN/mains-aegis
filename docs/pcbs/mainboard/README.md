...
- `U10.VREF` 通过 `R21=10kΩ` 连接到 `U10.PROG`；`R24=1MΩ` 从 `PROG` 接到 `CHGND`

#### `12V / 3A` 输入直通路径压降基线

以下为实板在 `12V / 3A` 条件下测得的分段压降。测量温度与仪表不确定度未记录，因此这些数据用于定位主要压降来源，不作为器件极限或量产公差：

| 区段 | 实测压降 | `3A` 下等效电阻 |
|---|---:|---:|
| 输入接口到 `R16` | `1mV` | `0.33mΩ` |
| `R16` 到 `Q22` 的 PCB 路径 | `1.6mV` | `0.53mΩ` |
| `Q22` | `5.7mV` | `1.90mΩ` |
| `Q22` 到 `Q11` 的 PCB 路径 | `3.6mV` | `1.20mΩ` |
| `Q11` | `18mV` | `6.00mΩ` |
| `Q11` 到输出接口 | `2.2mV` | `0.73mΩ` |

上述分段合计为 `32.1mV / 10.7mΩ`，不包含 `R16` 本体。`R16=7mΩ` 在 `3A` 下的理论压降约为 `21mV`；计入后，输入接口到输出接口的估算总压降约为 `53.1mV`，等效电阻约 `17.7mΩ`，路径损耗约 `159mW`。已测分段中 `Q11` 是最大的单项压降来源。
