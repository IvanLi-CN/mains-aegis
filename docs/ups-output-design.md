...
当前主板样机的板级排查已经验证到一个直接现象：若同时装配 A 侧补偿网络（`R71/C96/C97`）与 B 侧补偿网络（`R85/C128/C129`），再通过 `R90=0Ω` 把两侧 `COMP` 硬短，则单独调试 A 通道时会出现异常；断开 `R90` 后，A 通道在 `12V -> 5V` 条件下的 `SW1`、`DR1H-SW1`、`DR1L-GND` 波形恢复为正常 buck PWM。  
因此，当前样机调试结论应写死为：**单通道排查时保持 `R90` 断开；只有在并联联调时，且确认只保留一套共享补偿网络后，才允许重新接通 `COMP` 共点。**

### 4.2.1 当前样机联调笔记（冻结调试口径）

针对当前并联样机，推荐把 `COMP` 引脚环路补偿网络（`Rcomp/Ccomp/Cff`）作为明确的调试变量，而不是把两侧补偿网络默认都焊上再直接共点：

- 单通道排查起步时，`R90` 保持断开，A/B 各自补偿网络独立验证。
- 单路验证建议先在 `FPWM` 下覆盖 `5V / 12V / 15V / 19V` 的空载与轻载纹波。
- 单路稳定后，再装 `R90` 做并联联调；若需要共享 `COMP`，优先评估“只保留一套共享补偿网络”的装配方式。
- 当前样机最终收敛的可行装配是：
  - 外置 buck MOS gate 串联电阻保持 `0Ω`
  - `SW` 节点 RC snubber 保持 `DNP`
  - B 通道 `COMP` 引脚补偿网络保持 `DNP`

这组结论是**当前样机的板级验证结果**，不等同于 TPS55288 的通用推荐装配。
