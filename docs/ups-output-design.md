...
#### 5.3.2 在“22µF 不超过 4 颗、100µF 不超过 3 颗”的限制下，如何配 MLCC

本项目对每路输出侧 MLCC 的数量与封装约束为：

- `22µF / 35V / 0805`（每路 ≤ 4 颗）
- `100µF / 50V / 1210`（每路 ≤ 3 颗）

在不指定具体料号的前提下，“有效电容（含 DC bias）”只能做经验估计。这里优先用公开 DC bias 曲线给出量级，避免“按 nominal 直接相加”导致严重低估所需数量：
...
据此可以直接解释“为什么看起来要很多 22µF”：在 19V 固件最坏点需要 `COUT_eff ≥ 74.8µF`（5.3.1），而 `22µF/35V/0805` 在 `19V` 时每颗只有 `~2µF`，即使装满 4 颗也只有 `~8µF`，仅从 `Vripple(CAP)` 公式会得到接近 `~1Vpp` 的纹波量级。因此若要把 `TPS_VOUT` 的 CAP 项纹波压到 `~100mVpp`，就必须引入“更高有效电容/更低阻抗”的器件（例如大封装 MLCC 或更适合储能的 bulk），而不能只堆 0805。
...
上述组合仅给出电容数量与纹波量级。布局上至少应保证：`0.1µF + 1µF` 以及 **至少 1~2 颗中等/大容量 MLCC** 与 `VOUT/PGND` 形成最短回路；其余电容再根据空间分配到 `TPS_VOUT` 侧的可用位置，并通过样机测量确认 `SW2/VOUT` 尖峰与 `TPS_VOUT` 纹波满足要求。

若 PCB 空间不允许把全部电容都紧贴 `VOUT/PGND`（例如部分 MLCC 距离较远或回流路径不可控），输出电容可按“就地高频去耦”与“母线低频储能/阻尼”分层部署：

- **IC 侧（`TPS_VOUT`）优先保证“就地高频去耦 + 基本稳定性”**：例如 `0.1µF + 1µF + 1~2` 颗中等/大容量 MLCC（如 `22µF/0805`，或空间允许时 `100µF/1210`）尽量贴近 `VOUT/PGND`（必要时放到背面并用成对过孔直连），其任务是抑制尖峰/振铃并闭合高 di/dt 回路。
